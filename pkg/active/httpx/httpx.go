package httpx

import (
	"bufio"
	"deifzar/asmm8/pkg/log8"
	"os"
	"os/exec"
)

func RunHttpx(domains []string, sdomains map[string][]string) {
	for _, d := range domains {
		writeTempFile(d, sdomains[d])
		cmd := exec.Command("httpx", "-l", "tempHttpx-"+d+".txt", "-silent", "-td", "-csv", "-o", "temp-"+d+".csv")
		err := cmd.Run()
		if err != nil {
			log8.BaseLogger.Debug().Msg(err.Error())
			log8.BaseLogger.Panic().Msg("Httpx run failed")
			panic(err)
		}
		log8.BaseLogger.Info().Msg("Httpx run completed")
		// processCSV()
		os.Remove("tempHttpx-" + d + ".txt")
		// os.Remove("temp-" + d + ".csv")
	}
}

func writeTempFile(suffix string, list []string) {
	file, err := os.OpenFile("tempHttpx-"+suffix+".txt", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)

	if err != nil {
		log8.BaseLogger.Debug().Msg(err.Error())
		log8.BaseLogger.Panic().Msg("Write temp file for HTTPx results failed.")
		panic(err)
	}

	datawriter := bufio.NewWriter(file)

	for _, data := range list {
		_, _ = datawriter.WriteString(data + "\n")
	}

	datawriter.Flush()
	file.Close()
}

// func processCSV() {
// 	// Open the CSV file
// 	inputFile, err := os.Open("temp.csv")
// 	if err != nil {
// 		panic(err)
// 	}
// 	defer inputFile.Close()

// 	// Create a new CSV reader
// 	reader := csv.NewReader(inputFile)

// 	// Read the whole CSV into memory
// 	records, err := reader.ReadAll()
// 	if err != nil {
// 		panic(err)
// 	}

// 	// Specify the indices of the columns to keep
// 	columnsToKeep := []int{0, 1, 7, 8, 10, 13, 17, 20, 26, 27, 28, 32, 33, 35, 37} // Keeping only the first and third columns (0-indexed)

// 	// Open the output CSV file
// 	outputFile, err := os.Create("EasyEASM.csv")
// 	if err != nil {
// 		panic(err)
// 	}
// 	defer outputFile.Close()

// 	writer := csv.NewWriter(outputFile)
// 	defer writer.Flush()

// 	// Iterate through the records, keeping only the specified columns, and write to the output file
// 	for _, record := range records {
// 		var filteredRecord []string
// 		for _, columnIndex := range columnsToKeep {
// 			if columnIndex < len(record) {
// 				filteredRecord = append(filteredRecord, record[columnIndex])
// 			}
// 		}
// 		if err := writer.Write(filteredRecord); err != nil {
// 			panic(err)
// 		}
// 	}

// 	// Ensure everything is written to the output file
// 	if err := writer.Error(); err != nil {
// 		panic(err)
// 	}

// }
